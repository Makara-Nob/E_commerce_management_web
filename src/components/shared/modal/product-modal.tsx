...
const relatedProductSchema = z.object({
  product: z.any().nullable().refine((val) => val !== null, "Product is required"),
});
      ...
      if (product) {
        form.reset({
          ...
          relatedProducts: (product.relatedProducts || []).map((item) => ({
            product: typeof item === "object" && item !== null ? item : { id: item, name: item },
          })),
        });
      } else {
        form.reset({
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                <div className="sticky top-0 z-10 bg-background/95 backdrop-blur px-6 py-4 border-b mb-4">
